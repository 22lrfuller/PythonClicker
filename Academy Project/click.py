...
def buttonPress():
    ...
    if(event.type == pygame.MOUSEBUTTONDOWN):
        ...
        if(switch == 0):
            ...
            #Click Upgrade
            if(addingClick.collidepoint(pos) and menuShow == True):
...
def buyingButton(price, buttonType):
    ...
    global switch
    global buyMultiVar
    if(buttonType == 1 and round(int(clicks)) >= (round(price) * buyMultiVar)):
        switch = 1
        return 1
    if(buttonType == 2 and round(int(clicks)) >= (round(cpcPrice) * buyMultiVar)):
        switch = 1
        return 1
    else:
        screen.blit(font.render("Not Enought Money!", True, BLACK), [105, 130])
        if(event.type == pygame.MOUSEBUTTONUP):
            switch = 1
            screen.blit(font.render("Not Enought Money!", False, BLACK), [200, 150])
        return 0
...
while not done:
    ...
    menuButtonName = font.render("Menu", True, BLACK)
    backButtonName = font.render("Back", True, BLACK)
    buttonPrice1 = font.render(int(round(price1)), True, BLACK)
    ...
    if(menuShow == False):
        if(multiButtonNum == 1):
            buyOneText = font.render("Buy One", True, BLACK)
            screen.blit(buyOneText, [300, 250])
        ...
        if(multiButtonNum == 2):
            buyFiveText = font.render("Buy Five", True, BLACK)
            screen.blit(buyFiveText, [300, 250])

            screen.blit(str(round(buttonPrice1 * (1.15 ** 5))), [553, 22])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 97])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 172])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 247])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 322])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 397])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 472])
            screen.blit(round(buttonPrice1 * 1.15 ** 5), [553, 547])

        if(multiButtonNum == 3):
            buyTenText = font.render("Buy Ten", True, BLACK)
            screen.blit(buyTenText, [300, 250])

            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 22])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 97])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 172])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 247])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 322])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 397])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 472])
            screen.blit(round(buttonPrice1 * 1.15 ** 10), [553, 547])
        ...
        clickButton = pygame.draw.rect(screen, RED, (100, 150, 200, 200))
        menuButton = pygame.draw.rect(screen, ORANGE, (0, 675, 550, 75))
        ...
        #Other Button Names
        screen.blit(clickMe, [160, 240])
        screen.blit(clickText, [0, 0])
        screen.blit(clickUpgrade, [0, 20])
        screen.blit(menuButtonName, [250, 700])
    ...
    if(menuShow == True):
        ...
        screen.blit(backButtonName, [25, 40])
        screen.blit(buttonAddingClick, [380, 340])
        screen.blit(buttonAddingClick2, [400, 360])
        screen.blit(priceForCpc, [415, 400])
